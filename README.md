...
> [!IMPORTANT]
> Do the whole lab on your **VM**. You can work through a plain SSH shell or through `VS Code` Remotasdasde-SSH. When thisss guide says `localhost`, it means the VM itself or a forwarded port from that VM. Do not install or run `nanobot` on your main machine.
